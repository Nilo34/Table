...
public:
    ...
    virtual double Evaluate(const std::function<double(Position pos)>& args) const = 0;
...
public:
    ...
    double Evaluate(const std::function<double(Position pos)>& args) const override {
        ...
        switch (type_) {
            ...
            default:
                // Не совсем понятно какое должно быть выброшенно исключение из заранее прописанных
                // в common.h, поэтому будет выброшенно стандартное, подходящее по смыслу
                // хотя по сути мне кажется что это всё ещё ошибка формулы
    ...
    double Evaluate(const std::function<double(Position pos)>& args) const override {
        ...
        switch (type_) {
            ...
            case UnaryMinus:
                return -operand_->Evaluate(args);
            default:
                // Аналогично случаю с бинарными операциями 
                throw std::invalid_argument("Incorrect type of operation");
        }
    }
...
double FormulaAST::Execute(const std::function<double(Position pos)>& args) const {
